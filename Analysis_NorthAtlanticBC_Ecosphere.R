...
tl_v_bv_plot <- ggplot(all_data, aes(x = Total_Length, y = Body_Volume)) + # all historical data with total length as x and body volume as y 
  geom_point(data = rbind(husavik_comb, ww_comb), aes(x = Total_Length, y = Body_Volume, colour = "Iceland"), size = 2, alpha = 0.75) + # icelandic data combined and loaded in with the assigned color 
  geom_point(data = nyb_comb, aes(x = Total_Length, y = Body_Volume, colour = "New York Bight"), size = 2, alpha = 0.75) + # NYB data loaded in with assigned color 
  geom_point(data = rbind(aw_comb,unb_comb), aes(x = Total_Length, y = Body_Volume, colour = "Gulf of Maine"), size = 2, alpha = 0.75) + # AW and UNB data loaded in with assigned coor 
  geom_point(data = gl_comb, aes(x = Total_Length, y = Body_Volume, colour = "Greenland"), size = 2, alpha = 0.75) + # greenland data loaded in with the assigned color 
